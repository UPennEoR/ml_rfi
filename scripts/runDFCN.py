...
f1 = 2.0 * precision[0] * recall[0] / (precision[0] + recall[0])
...
dset.load(
    ...
    hybrid=hybrid,
    chtypes=chtypes,
    fold_factor=f_factor,
    cut=cut,
    patchwise_train=patchwise_train,
    expand=expand,
)
...
with tf.Session(config=config) as sess:
    ...
    if mode == "train":
        ...
        for i in range(start_step, start_step + num_steps + 1):
            ...
            feed_dict = {
                vis_input: batch_x,
                RFI_targets: batch_targets,
                learn_rate: lr,
                mode_bn: True,
                d_out: dropout,
            }
            _, loss_, s1, rec, pre, f1_, ba = sess.run(
                [train_fcn, loss, summary, recall, precision, f1, batch_accuracy],
                feed_dict=feed_dict,
            )
            if i % 20 == 0:
        ...
        eval_writer = tf.summary.FileWriter(
            "./" + model_name + "_eval_" + vdset + "/", sess.graph
        )
        for i in range(start_step, start_step + num_steps + 1):
            batch_x, batch_targets = dset.next_eval()
            feed_dict = {vis_input: batch_x, RFI_targets: batch_targets, mode_bn: True}
            eval_class, rec, pre, f1_, s1, loss_ = sess.run(
                [RFI_guess, recall, precision, f1, summary, loss], feed_dict=feed_dict
            )
            ...
            if i % 10 == 0:
                ...
                acc = hf.batch_accuracy(
                    batch_targets, tf.argmax(eval_class, axis=-1)
                ).eval()
        ...
        for i in range(start_step, start_step + num_steps + 1):
            ...
            for j in range(int(dset.get_size() / batch_init)):
                ...
                feed_dict_train = {
                    vis_input: batch_x_train,
                    RFI_targets: batch_targets_train,
                    learn_rate: lr,
                    mode_bn: True,
                }
                _, loss_, strain, rec, pre, f1_train, ba = sess.run(
                    [train_fcn, loss, summary, recall, precision, f1, batch_accuracy],
                ...
                feed_dict_eval = {
                    vis_input: batch_x_eval,
                    RFI_targets: batch_targets_eval,
                    mode_bn: True,
                }
                eval_class, loss_eval, rec, pre, f1_eval, seval = sess.run(
                    [RFI_guess, loss, recall, precision, f1, summary],
            ...
            print(
                "Epoch {0} | Train loss : {1} ---- Validation loss : {2}".format(
                    i, np.mean(train_loss), np.mean(val_loss)
                )
            )
            print(
                "            Train F1   : {1} ---- Validation F1   : {2}".format(
                    i, np.mean(f1_train_arr), np.mean(f1_val_arr)
                )
            )
            if i % 100 == 0 and i != 0:
        ...
        while ct != 18:
            ...
            pred_unfold = hf.unfoldl(
                tf.reshape(tf.argmax(g, axis=-1), [16, ps, ps]).eval(), padding=16
            )
            # pred_unfold = hf.unfoldl(tf.reshape(g[:,:,1],[16,68,68]).eval())
            pred_unfold_0 = hf.unfoldl(
                tf.reshape(g[:, :, 0], [16, ps, ps]).eval(), padding=16
            )
            pred_unfold_1 = hf.unfoldl(
            ...
            mcc_arr.append(mcc)
            acc_arr.append(acc)
            f2_arr.append(5.0 * tpr * fpr / (4.0 * fpr + tpr))
            # Save individual visibility samples that have been predicted on
            if waterfall_sample:
                np.savez(
                    "Real_{0}_SamplePredict_{1}.npz".format(chtypes, ct),
                    data=data_,
                    target=target_unfold,
                    prediction=pred_unfold_,
                    f2=5.0 * tpr * fpr / (4.0 * fpr + tpr),
                    recall=tpr,
                    precision=fpr,
                )
            ...
            ct += 1
            if stats:
                FPR_, TPR_, MCC_, F2_, best_thresh = hf.ROC_stats(
                    targets_, predicts_.reshape(1, -1)
                )
                ...
                np.savez(
                    "ROC_curves_newSim_{0}.npz".format(chtypes),
                    TPR=_TPR_ARR,
                    FPR=_FPR_ARR,
                    MCC=_MCC_ARR,
                    F2=_F2_ARR,
                    best_thresh=np.nanmedian(best_thresh_arr),
                )
        ...
        if ROC:
            ...
            try:
                ...
                f.close()
        print(
            "Prediction pipeline time per waterfall visibility: {0}".format(
                pred_time + dset_load_time
            )
        )
